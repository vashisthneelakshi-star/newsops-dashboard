...
// Parse date formats: "01.11.25" or "2026-05-25"
function parseDate(raw) {
  ...
  if (raw.includes("-")) {
    ...
    if (parts.length === 3) {
      const m = parts[1].padStart(2, "0");
      return { date: raw, month: MONTH_NAMES[parseInt(m)] || "" };
    }
  }
  return { date: raw, month: "" };
}

// Parse month: "11" → "November", "May" → "May"
function parseMonth(raw) {
  if (!raw) return "";
  raw = raw.trim();
  const num = parseInt(raw);
  if (!isNaN(num) && num >= 1 && num <= 12) return MONTH_NAMES[num];
  return raw; // already a name like "May"
}

// Parse time: "10/31/2025 22:00" or "22:00" → "22:00"
...
export default function App() {
  ...
  // KPI for currently selected view
  const kpi = useMemo(() => {
    const late = base.filter(r => r.dm > 0);
    return {
      total: base.length,
      late: late.length,
      early: base.filter(r => r.dm < 0).length,
      ontime: base.filter(r => r.dm === 0).length,
  ...
  const handleCSV = e => {
    ...
    rd.onload = ev => {
      ...
      const rows = lines.slice(1).map(ln => {
        ...
        const parsedMonth = parseMonth(rawMonth) || monthFromDate;

        // Parse times
        const st = parseTime(row["scheduletime"] || row["schedule_time"] || row["scheduledtime"] || "");
        const rt = parseTime(row["releasetime"]  || row["release_time"]  || row["actualtime"]    || "");
      ...
      if (rows.length) {
        setData(rows); // Replace all data with new CSV
        setShowUp(false);
        // Auto-set month filter to first record's month
  ...
  const exportCSV = () => {
    ...
    const rows = base.map(r => {
      const d = fmtD(r.dm);
      return [r.month,r.date,r.state,r.branch,r.edition,r.pullout,r.st,r.rt,d.label,r.cause].join(",");
    });
  ...
  const Pill = ({ label, active, onClick }) => (
    <button onClick={onClick} style={{ padding: "5px 14px", borderRadius: 20, border: `1.5px solid ${active ? "#185fa5" : "#d0d0d0"}`, background: active ? "#185fa5" : "transparent", color: active ? "#fff" : "#666", fontWeight: active ? 500 : 400, fontSize: 12, cursor: "pointer" }}>
      {label}
    </button>
  );
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", background: "#fff", borderRadius: 14, padding: "20px 24px", boxShadow: "0 1px 4px rgba(0,0,0,.08)" }}>
        ...
        {showUp && (
          <div style={{ background: "#f8f9fa", border: "1px solid #e0e0e0", borderRadius: 9, padding: "14px 16px", marginBottom: 16, fontSize: 12 }}>
            ...
            <div style={{ color: "#888", marginBottom: 8 }}>
              Supported formats: <strong>Tab-separated (.xls save as TSV) or Comma-separated (.csv)</strong><br/>
              Columns: Month, Date, State, Branch, Edition, Pullout, Schedule Time, Release Time, Delay Cause<br/>
              Date format supported: <code>01.11.25</code> or <code>2025-11-01</code><br/>
              Time format supported: <code>10/31/2025 22:00</code> or <code>22:00</code>
            </div>
            <input type="file" accept=".csv,.tsv,.txt" onChange={handleCSV} />
            <div style={{ marginTop: 8, color: "#a32d2d", fontSize: 11 }}>⚠️ Importing will replace all current data</div>
          </div>
        )}
        ...
        {data.length === 0 && (
          <div style={{ textAlign: "center", padding: "40px 20px", color: "#aaa" }}>
            <i className="ti ti-file-upload" style={{ fontSize: 48, display: "block", marginBottom: 12 }}></i>
            <div style={{ fontSize: 16, fontWeight: 500, marginBottom: 8, color: "#888" }}>No Data Loaded</div>
            <div style={{ fontSize: 13, marginBottom: 16 }}>Import your daily CSV/TSV sheet to get started</div>
            <button onClick={() => setShowUp(true)} style={{ padding: "8px 20px", background: "#185fa5", color: "#fff", border: "none", borderRadius: 8, fontSize: 13, cursor: "pointer" }}>
              Import CSV Sheet
            </button>
          </div>
        )}
        ...
        {data.length > 0 && (
          <>
              ...
              <div>
                ...
                <div style={{ overflowX: "auto", border: "1px solid #eee", borderRadius: 10 }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 700 }}>
                    ...
                    <tbody>
                      {base.length === 0 && (
                        <tr><td colSpan={8} style={{ padding: 24, textAlign: "center", color: "#aaa" }}>No data for this selection.</td></tr>
                      )}
                      {base.map((r, i) => {
            ...
            {tab === "states" && (
              <div>
                <div style={{ fontSize: 12, color: "#888", marginBottom: 12 }}>Click a state card to drill down into its editions</div>
            ...
            {tab === "top10" && (
              <div className="grid2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                <div style={{ border: "1px solid #f09595", borderRadius: 10, padding: 14 }}>
                  ...
                  {top10late.length === 0 && <div style={{ fontSize: 12, color: "#aaa" }}>No late editions in this selection</div>}
                  {top10late.map((r, i) => (
            ...
            {tab === "causes" && (
              <div>
                ...
                {causes.length === 0 && <div style={{ fontSize: 12, color: "#aaa" }}>No delay causes recorded in this selection.</div>}
